...
def add_ingredient():
    ...
    if form.validate_on_submit():
        ingredient_exists = Ingredient.query.filter(Ingredient.name == form.data['name']).one_or_none()
...
@ingredient_routes.route('/<int:recipe_id>/<int:ingredient_id>/add-recipe-ingredient', method=['POST'])
@login_required
def add_recipe_ingredient(recipe_id,ingredient_id):
    
    recipe_ingredient_exists = recipe_ingredients.query.filter(recipe_ingredients.recipe_id == recipe_id, recipe_ingredients.ingredient_id).one_or_none()
...
@ingredient_routes.route('/<int:recipe_id>/<int:ingredient_id>/delete-recipe-ingredient', method=['DELETE'])
@login_required
def delete_ingredient(recipe_id, ingredient_id):
    #Current Tag
    #Recipe tag relationship we want to delete
    recipe_ingredient_to_delete = recipe_ingredients.query.filter(recipe_ingredients.recipe_id == recipe_id, recipe_ingredients.ingredient_id == ingredient_id).one_or_none()
    ...
    ingredient_usage = recipe_ingredients.query.filter(recipe_ingredients.ingredient_id == ingredient_id).count()
    if ingredient_usage == 1:
        curr_ingredient = Ingredient.query.filter(Ingredient.id == ingredient_id).one_or_none()
        if curr_ingredient:
