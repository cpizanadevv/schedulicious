...
def add_ingredient():
    ...
    if form.validate_on_submit():
        ...
        if ingredient_exists:
            return ingredient_exists.to_dict(), 200
...
def delete_ingredient(recipe_id, ingredient_id):
    print("def")
    recipe_ingredient_to_delete = db.session.execute(
        select([recipe_ingredients])
            .where(recipe_ingredients.c.recipe_id == recipe_id)
            .where(recipe_ingredients.c.ingredient_id == ingredient_id)
    ).fetchone()
    print("ABC",recipe_ingredient_to_delete)
...
@ingredient_routes.route(
    "/<int:recipe_id>/delete-all-recipe-ingredient", methods=["DELETE"]
)
@login_required
def delete_all_recipe_ingredients(recipe_id):
    recipe_ingredient_to_delete = db.session.execute(
        select(recipe_ingredients).where(
            recipe_ingredients.c.recipe_id == recipe_id
    )).fetchall()
    
    for recipe_ingredient in recipe_ingredient_to_delete:
        db.session.execute(
