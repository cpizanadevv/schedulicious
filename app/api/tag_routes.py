...
def add_recipe_tag(recipe_id, tag_id):
    ...
    recipe_tag_exists = db.session.execute(
        select([recipe_tags]).where(
            recipe_tags.c.recipe_id == recipe_id, recipe_tags.c.tag_id == tag_id
        )
    ).fetchone()
...
def delete_recipe_tag(recipe_id, tag_id):
    ...
    recipe_tag_to_delete = db.session.execute(
        select([recipe_tags]).where(
            recipe_tags.c.recipe_id == recipe_id, recipe_tags.c.tag_id == tag_id
        )
    ).fetchone()
    ...
    delete_stmt = (
        db.Table.delete()
        .where(recipe_tags.c.recipe_id == recipe_id)
        .where(recipe_tags.c.tag_id == tag_id)
    )
